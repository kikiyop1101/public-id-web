...
function mountScrollWorld(container, config) {
  ...
  // 태그는 문자열이거나 {label, href}. href가 있으면 클릭 가능한 칩이 된다.
  // 절대 URL(다른 사이트)은 새 탭 — 스크롤 월드 몰입을 끊지 않기 위해서다.
  const tagItem = t => {
    if (typeof t === 'string') return `<li>${esc(t)}</li>`;
    const ext = /^https?:\/\//i.test(t.href || '');
    const attrs = ext ? ' target="_blank" rel="noopener noreferrer"' : '';
    return `<li><a class="sw-tag-link" href="${esc(t.href || '#')}"${attrs}>${esc(t.label || '')}</a></li>`;
  };

  const copies = [], dots = [], copyOps = [];
  SECTIONS.forEach((s, i) => {
    ...
    c.innerHTML =
      ...
      (s.tags && s.tags.length ? `<ul class="sw-copy__tags">${s.tags.map(tagItem).join('')}</ul>` : '') +
  ...
  function jumpTo(i) {
    // 첫 장면 카피는 '랜딩 인사말'이라 스크롤하면 사라지도록 만들어져 있다.
    // 그래서 세그먼트 중앙으로 보내면 카피가 이미 걷힌 자리라 빈 화면처럼 보인다
    // → 첫 장면만 맨 위로 (대표 지적 2026-07-26)
    if (i === 0) {
  ...
  function read() {
    ...
    // 장면 전환 중에는 카피가 두 장 겹친다. 임계를 느슨하게 두면 거의 투명한 쪽이
    // 클릭을 가로채, 화면에 보이는 것과 다른 곳으로 이동한다(대표 실사용 신고 2026-07-26
    // — "얼마나 빠르게 누르느냐에 따라 가는 데가 다르다").
    // → 가장 또렷한 카피 하나만, 그것도 충분히 또렷할 때만 클릭을 받는다.
    let topI = -1, topOp = 0;
    for (let i = 0; i < N; i++) if (copyOps[i] > topOp) { topOp = copyOps[i]; topI = i; }
    for (let i = 0; i < N; i++)
      copies[i].style.pointerEvents = (i === topI && topOp > 0.85) ? 'auto' : 'none';
...
function injectCSS() {
  ...
  .sw-copy__tags li:has(.sw-tag-link){padding:0;}
  .sw-tag-link{display:block;padding:7px 14px;color:inherit;text-decoration:none;border-radius:999px;transition:background .2s ease,transform .2s ease;}
  .sw-tag-link:hover{background:color-mix(in srgb,var(--sw-accent) 26%,#fff);transform:translateY(-1px);}
  .sw-tag-link:focus-visible{outline:2px solid var(--sw-accent);outline-offset:2px;}
